  ...
  register(options = {}) {
    ...
    return async (req, res, next) => {
      ...
      const attestation = new AttestationChallengeBuilder(this)
        ...
        .setUserVerification(this.config.userVerification)
  ...
  verifyAuthenticatorAttestationResponse(webauthnResponse) {
    ...
    if (response.verified) {
      response.authrInfo = {
        ...
        aaguid: authrDataStruct.aaguid,
        credID: base64url.encode(authrDataStruct.credID),
      }
      if (ctapMakeCredResp.attStmt !== undefined && ctapMakeCredResp.attStmt !== null) {
        response.authrInfo.attestation = {
          format: ctapMakeCredResp.fmt,
          alg: ctapMakeCredResp.attStmt.alg,
        }
      }
